...
class BaseModel(nn.Module):
    def __init__(self, in_channels, button_num, variable_num, frame_num, batch_size):
        ...
        self.conv5 = nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, stride=2)
        self.bn = nn.BatchNorm2d(256)
        self.fc1 = nn.Linear(256 * 2 * 3, 256)
        '''
        self.conv1 = nn.Conv2d(in_channels=in_channels, out_channels=32, kernel_size=5, stride=2)
        self.conv2 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=2)
        self.conv3 = ResBlock(32, 64, stride=2, padding=(1, 1))
        self.conv4 = ResBlock(64, 128, stride=2, padding=(1, 1))
        self.conv5 = ResBlock(128, 256, stride=2, padding=(1, 1))
        self.fc1 = nn.Linear(256 * 4 * 5, 256)
        '''
        ...
        self.labels1 = nn.ConvTranspose2d(256, 256, (2, 3), stride=2)
        self.labels2 = nn.ConvTranspose2d(256, 128, 3, stride=2)
        self.labels3 = nn.ConvTranspose2d(128, 8, 4, stride=2, padding=(2, 0))
    ...
    def forward(self, screen, variables, prev_action, cells, non_terminal, update_cells=True):
        ...
        screen_features = self.conv5(screen_features)
        screen_features = self.bn(screen_features)
        screen_features = F.relu(screen_features, inplace=True)
        combined_features = screen_features.view(screen_features.size(0), -1)
        combined_features = F.relu(self.fc1(combined_features), inplace=True)
        combined_features = torch.cat([combined_features, variables, prev_action], 1)
        '''
        screen_features = F.relu(self.conv1(screen), inplace=True)
        screen_features = F.relu(self.conv2(screen_features), inplace=True)
        screen_features = self.conv3(screen_features)
        screen_features = self.conv4(screen_features)
        screen_features = self.conv5(screen_features)
        screen_features = screen_features.view(screen_features.size(0), -1)
        screen_features = F.relu(self.fc1(screen_features), inplace=True)
        combined_features = torch.cat([screen_features, variables, prev_action], 1)
        '''
        # rnn
        if screen_features.shape[0] <= self.batch_size:
            data = cells.get_masked(non_terminal)
            data = self.screen_features1(combined_features, data)
            if update_cells:
                cells.data = data
            return data[0], screen_features
        else:
            features = []
            for i in range(combined_features.shape[0]//cells.batch_size):
                data = cells.get_masked(non_terminal[i])
                start = i * cells.batch_size
                cells.data = self.screen_features1(combined_features[start:start+cells.batch_size], data)
                features.append(cells.data[0])
            features = torch.cat(features, dim=0)
            return features, screen_features
...
StepInfo = namedtuple('StepInfo', ['screen', 'variables', 'prev_action', 'log_action', 'value', 'action', 'labels'])
...
class PPOScreen(PPOBase):
    ...
    def forward(self, screen, variables, prev_action, non_terminals, action_only=False, save_step_info=False, action=None, labels=None):
        features, screen_features = self.model.forward(screen, variables, prev_action, self.cells, non_terminals)
        ...
        if save_step_info:
            ...
            self.steps.append(StepInfo(screen.cpu(), variables, prev_action, action_log_prob, value, action, labels))

        return action, action_log_prob, value, entropy, screen_features

    def get_action(self, state, prev_action):
        with torch.set_grad_enabled(False):
            action, _, _ = self.forward(
                *self.model.transform_input(state.screen, state.variables, prev_action), self.non_terminal, action_only=True
            )
        return action

    def get_save_action(self, state, prev_action):
        with torch.set_grad_enabled(False):
            action, _, _, _, _ = self.forward(
                *self.model.transform_input(state.screen, state.variables, prev_action), self.non_terminal, save_step_info=True, labels=state.labels
            )
        return action
    ...
    def backward(self):
        ...
        #labels = torch.cat([step.labels for step in self.steps], dim=0).to(device)
        for batch in range(2):
            ...
            for sub_batch_start in range(0, self.args.batch_size, sub_batch_size):
                ...
                _, log_actions, values, entropy, screen_features = self.forward(screens[r1:r2], variables[r1:r2], prev_actions[r1:r2], non_terminals[r1:r2], action=actions[r1:r2])
                ...
                #labels_pred = F.relu(self.model.labels1(screen_features))
                #labels_pred = F.relu(self.model.labels2(labels_pred))
                #labels_pred = self.model.labels3(labels_pred)
                #labels_pred = F.log_softmax(labels_pred, dim=1)
                #labels_loss = F.nll_loss(labels_pred, labels[r1:r2])
                ...
                loss = policy_loss + value_loss # + labels_loss #+ entropy_loss #+ 0.0001*weights_l2
                # sub batch weight
